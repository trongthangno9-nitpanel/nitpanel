...
yum update curl wget ca-certificates -y 1> /dev/null
        ...
        echo -e "\nNitPanel is supported on Ubuntu 18.04, Ubuntu 20.04, Ubuntu 22.04, AlmaLinux 8, AlmaLinux 9, AlmaLinux 10, Rocky Linux 8, Rocky Linux 9 and CloudLinux 7.x...\n"
        exit 1
fi

rm -f nitpanel.sh
rm -f install.tar.gz
curl --silent -o nitpanel.sh "https://nitpanel.netihot.com/dl/$SERVER_OS" 2>/dev/null
chmod +x nitpanel.sh
./nitpanel.sh $@
